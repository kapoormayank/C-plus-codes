// Maximum Depth Of BST Algorithm Implementation In C++
// Time Complexity: O(n)
...
int maximumDepth(Node* root) {
    ...
    // recursive function
    int leftheight=maximumDepth(root->left); // Left Subtree
    int rightheight=maximumDepth(root->right); // Right Subtree
...
int main() {
    ...
    print(root);
    cout<<"\nMaximum Depth of Binary Search Tree(BST): "<<maximumDepth(root)<<endl;
    return 0;
}
